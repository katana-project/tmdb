...
def unescape_json(v: str, item_path: str | None = None) -> dict[str, any]:
    if len(v) > 2 and (v.startswith("{}") or v.startswith("[]")):  # check malformed JSON example value
        if item_path is not None:
            print(f"fixed malformed example, path: {item_path}")
...
def clean_schema_tree(node: list | dict[str, any], path: str = ""):
    if isinstance(node, dict):
        for k, v in list(node.items()):
            ...
            if isinstance(v, dict):
                ...
                if k == "properties":
                    ...
                    for k1, v1 in sorted(v.items(), reverse=True, key=(lambda e: e[0])):
                        ...
                        if clean_key in keys:
                            ...
                            print(f"fixed duplicate property {k1} -> {clean_key}, path: {item_path}.{k1}")
                        else:
                    ...
                    print(f"fixed vote-related property type integer -> number, nullable, path: {item_path}")
                    ...
                    print(f"fixed non-null property type, path: {item_path}")
                    ...
                    print(f"fixed missing schema, path: {item_path}")
                ...
                print(f"fixed null default, path: {item_path}")
            elif ".examples" in item_path and k == "value" and isinstance(v, str):  # check escaped JSON example value
                if (v.startswith("{") and v.endswith("}")) or (v.startswith("[") and v.endswith("]")):
                    node[k] = unescape_json(v, item_path=item_path)

                    print(f"fixed escaped example, path: {item_path}")
    else:
        for i, v in enumerate(node):
            if isinstance(v, dict) or isinstance(v, list):
                clean_schema_tree(v, path=f"{path}.{i}")
